...
/*
    This code has been modified by me, Martin Wass, to utilise GPU accelerator programming with OpenACC.
    My modifications can be found by searching for "@wass" in this document
*/
...
// @wass: I defined these constants for various sizes used in the code.
...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  /*
    ...
    state_out and state_init are sometimes both pointing to the same memory. I found it difficult to use "copyin" and "copyout"
  */
  #pragma acc parallel loop \
              collapse(3) \
              copyin(tend[0:TEND_SIZE]) \
              copy(state_out[0:STATE_SIZE], state_init[0:STATE_SIZE]) \
              private(inds, indt)
...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  */
  #pragma acc parallel loop \
              collapse(2) \
              copyin(state[0:STATE_SIZE], hy_dens_cell[0:HYDENSCELL_SIZE], hy_dens_theta_cell[0:HYDENSTHETACELL_SIZE]) \
...
void compute_tendencies_z( double *state , double *flux , double *tend ) {
  ...
  */
  #pragma acc parallel loop \
              collapse(2) \
              copyin(hy_dens_int[0:HYDENSINT_SIZE], hy_dens_theta_int[0:HYDENSTHETAINT_SIZE], state[0:STATE_SIZE]) \
              copyout(flux[0:FLUX_SIZE]) \
              private(ll, s, inds, stencil, vals, d3_vals, r, u, w, t, p)
  for (k=0; k<nz+1; k++) {
    for (i=0; i<nx; i++) {
  ...
  */
  #pragma acc parallel loop \
              collapse(3) \
              copyin(state[0:STATE_SIZE], flux[0:FLUX_SIZE]) \
              copyout(tend[0:TEND_SIZE]) \
              private(indt, indf1, indf2)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
...
void set_halo_values_x( double *state ) {
  ...
  */
  #pragma acc parallel loop \
              ...
              copyout(sendbuf_l[0:SENDBUF_SIZE], sendbuf_r[0:SENDBUF_SIZE])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
  ...
  /*
    ...
    I found it difficult to use "copyin" and "copyout" on state since it is not indexed contiguously
  */
  #pragma acc parallel loop \
              collapse(3) \
              copyin(recvbuf_l[0:RECVBUF_SIZE], recvbuf_r[0:RECVBUF_SIZE]) \
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      ...
      */
      #pragma acc parallel loop \
                  collapse(2) \
                  copyin(hy_dens_cell[0:HYDENSCELL_SIZE], hy_dens_theta_cell[0:HYDENSTHETACELL_SIZE]) \
                  copy(state[0:STATE_SIZE]) \
