...
#define RECVBUF_SIZE (hs*nz*NUM_VARS)
#define HYDENSCELL_SIZE (nz+2*hs)
#define HYDENSTHETACELL_SIZE (nz+2*hs)
#define HYDENSINT_SIZE (nz+1)
#define HYDENSTHETAINT_SIZE (nz+1)
#define HYPRESSUREINT_SIZE (nz+1)
...
/* FUNCTION FOR MINIMUM OF TWO DOUBLE VALUES */
inline double dmin(double a, double b) {
    return (a < b) ? a : b;
};

/* FUNCTIONS WHICH ARE DEFINED AFTER "main" */
...
int main(int argc, char **argv) {
    ...
    /* BEGIN USER-CONFIGURABLE PARAMETERS */
        ...
        sim_time = /*1500*/ 50;     //How many seconds to run the simulation
    ...
    /* LONG STATEMENT */
    {
        ...
        /* MAIN TIME STEP LOOP */
        /*
            Create new data region, spanning the while-loop
            Create the arrays "flux", "tend" and "state_tmp" on the device. They never need to leave the device
            Copy in the values of "hy_dens_cell" and "hy_dens_theta_cell" which were set in "init" function 
        */
        #pragma acc data \
                    create(flux[0:FLUX_SIZE], tend[0:TEND_SIZE], state_tmp[0:STATE_SIZE], sendbuf_l[0:SENDBUF_SIZE], sendbuf_r[0:SENDBUF_SIZE], recvbuf_l[0:RECVBUF_SIZE], recvbuf_r[0:RECVBUF_SIZE]) \
                    copyin(hy_dens_cell[0:HYDENSCELL_SIZE], hy_dens_theta_cell[0:HYDENSTHETACELL_SIZE], hy_dens_int[0:HYDENSINT_SIZE], hy_dens_theta_int[0:HYDENSTHETAINT_SIZE], hy_pressure_int[0:HYPRESSUREINT_SIZE], state[0:STATE_SIZE])
        {
            while(etime < sim_time) {
                ...
                /*
                    The array "state" needs to be copied out of the device every timestep so that it can be used in function "output"
                    TODO: Is it enough with just copyout?
                */
...
void semi_discrete_step(double *state_init, double *state_forcing, double *state_out, double dt, int dir, double *flux, double *tend) {
    ...
    #pragma acc update host(tend[0:TEND_SIZE])
    for(int i = 0; i < TEND_SIZE; i++) assert(!isnan(tend[i]));

    //Apply the tendencies to the fluid state
    /*
        The parallel region uses the existing "state_out", "state_init" and "tend". We can collapse the loops since they are tightly nested
    */
    #pragma acc parallel loop \
                collapse(3) \
                present(state_out, state_init, tend) \
                private(inds, indt)
...
void compute_tendencies_x(double *state, double *flux, double *tend) {
    ...
    /*
        Collapse loops, use present "state" and "flux"
    */
    #pragma acc parallel loop \
                collapse(2) \
                present(state, hy_dens_cell, hy_dens_theta_cell, flux) \
                private(ll, s, inds, stencil, vals, d3_vals, r, u, w, t, p)
    for(k=0; k<nz; k++) {          ///////// COLLAPSED
        for(i=0; i<nx+1; i++) {   /////////  COLLAPSED
            ...
            /* If I remove these statements, I get a lot of NAN-values in the output. My understanding is that vals-array is filled with nan-values */
            vals[0] = 1;
            vals[1] = 1;
            vals[2] = 1;
            vals[3] = 1;
    ...
    /*
        Collapse the loop, use present "flux" and "tend"
    */
    #pragma acc parallel loop \
                collapse(3) \
                present(tend, flux) \
...
void compute_tendencies_z(double *state, double *flux, double *tend) {
    ...
    //Compute fluxes in the x-direction for each cell
    /*
        Collapse loop, use present "flux" and "state"
    */
    #pragma acc parallel loop \
                collapse(2) \
                present(state, flux, hy_dens_int, hy_dens_theta_int, hy_pressure_int) \
                private(inds, r, u, w, t, p)
    for(k=0; k<nz+1; k++) {
        for(i=0; i<nx; i++) {
            //Use fourth-order interpolation from four cell averages to compute the value at the interface in question
            for(ll=0; ll<NUM_VARS; ll++) {
    ...
    #pragma acc parallel loop \
                collapse(3) \
                present(tend, flux, state) private(indt, indf1, indf2, inds)
    for(ll=0; ll<NUM_VARS; ll++) {
        for(k=0; k<nz; k++) {
            for(i=0; i<nx; i++) {
                indt  = ll* nz   * nx    + k* nx    + i  ;
...
void set_halo_values_x(double *state) {
    ...
    /*
        Copy out "sendbuf_l" and "sendbuf_r" since they are used in the MPI_Isend
    */
    #pragma acc parallel loop \
                collapse(3) \
    ...
    if(data_spec_int == DATA_SPEC_INJECTION) {
        ...
        #pragma acc parallel loop \
                    collapse(2) \
                    present(state, hy_dens_cell, hy_dens_theta_cell) private(z, ind_r, ind_u, ind_t)
        for(k=0; k<nz; k++) {
            for(i=0; i<hs; i++) {
            z = (k_beg + k+0.5)*dz;
...
void set_halo_values_z(double *state) {
  ...
  #pragma acc parallel loop collapse(2) \
              present(state) private(x, xloc, mnt_deriv)
  for(ll=0; ll<NUM_VARS; ll++) {
    for(i=0; i<nx+2*hs; i++) {
      if(ll == ID_WMOM) {
